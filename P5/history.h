...
struct Cmd {
	...
	int exitStatus; //The exit status from the user’s command
};
//Define the maximum number of entries in the history array and max command length allowed
#define MAX_LENGTH 1000 // Defining the max length a command may be
...
void print_history(int firstSequenceNumber); //Prints the history to stdoutgma once
